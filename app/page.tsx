...
const heroImages = [
  ...
  'makise-kurisu-1.webp', 'atam-2.webp', 'kintaro-1.webp',
]
...
export default function Home() {
  ...
  return (
    <main className={light ? 'site light' : 'site'}>
      ...
      <header className={scrollY > 24 ? 'nav scrolled' : 'nav'}>
      ...
      <section id="about" className="about section-grid content-section">
        <div className="section-label reveal"><small>[001]</small><h2>ABOUT</h2><div className="identity"><span className="identity-dot">♙</span><small>KINTARO<br />DEVELOPER</small></div></div>
        <div className="about-copy"><h3 className="reveal">I&apos;m a <em>Full Stack Developer</em><br />focused on building <b>clean and<br />sustainable systems.</b></h3><p className="reveal">I&apos;m someone who loves <b>learning new things</b> and constantly tries to put what I learn into practice. I develop <i>web and desktop applications</i>. I enjoy working with simple, practical, and <b>sustainable tools.</b></p><button className="underline-button reveal" onClick={() => setExpanded(!expanded)}>Read Full Version <Arrow /></button>{expanded && <p className="expanded">I care about thoughtful interfaces, maintainable code, and tools that stay useful for years.</p>}</div>
      </section>

      <section id="stack" className="stack content-section">
        <div className="section-heading reveal"><small>[002]</small><h2>STACK</h2></div>
        <div className="stack-groups">{stackGroups.map((group) => <div className="stack-group reveal" key={group.number}><div className="group-title"><span>{group.number}</span><h3>{group.title}</h3></div><div className="stack-items">{group.items.map((item, i) => { const icon = group.icons[i]; return <div className="stack-item" key={item}><img src={`/assets/stack/${icon}.${icon === 'neon' ? 'png' : 'svg'}`} alt="" />{item}</div> })}</div></div>)}</div>
      </section>

      <div className="ticker"><span>{ticker}</span><span>{ticker}</span></div>

      <section id="projects" className="projects content-section">
        <div className="projects-intro reveal"><small>[003]</small><h2>PROJECTS</h2><p>A collection of <em>experiments,</em><br /><em>products,</em> and <em>digital<br />artifacts</em> forged in the <b>void.</b></p><small className="scroll-label">SCROLL TO EXPLORE</small></div>
        <div className="project-list">{projects.map((project) => <article className="project-card reveal" key={project.title}><div className="project-image"><img src={project.image} alt={project.title} /><div className="project-overlay"><span>VIEW PROJECT</span><Arrow /></div></div><div className="project-meta"><span>{project.type}</span><span>{project.year}</span></div><h3>{project.title}</h3></article>)}</div>
      </section>

      <div className="ticker"><span>{ticker}</span><span>{ticker}</span></div>

      <section id="roadmap" className="roadmap content-section">
        <div className="center-heading reveal"><small>[004]</small><h2>ROADMAP</h2><p>A roadmap where I share the experiences I&apos;ve gained throughout my<br />software journey and the technologies I&apos;ve learned.</p></div>
        <div className="timeline reveal-line">{roadmap.map((item, i) => <article className="timeline-item reveal" key={item.year}><div className="timeline-dot" /><div className="timeline-card"><small>0{i + 1}</small><h3>{item.year}</h3><p>{item.text}</p><div className="tags">{item.tags.map(tag => <span key={tag}>{tag}</span>)}</div></div></article>)}</div>
      </section>

      <section id="contact" className="contact content-section">
        <div className="center-heading reveal"><small>[005]</small><h2>CONTACT</h2><p>Whether we start fresh to bring a project to life or take an existing system<br />further.</p></div>
        <div className="contact-links reveal"><a href="mailto:mustafw42@gmail.com"><span>SEND AN EMAIL</span><b>mustafw42@gmail.com</b><Arrow /></a><a href="tel:Notaddedyet"><span>DIRECT LINE</span><b>Not added yet.</b><Arrow /></a></div>
        <footer className="reveal"><span>© 2026 KINTARO. All rights reserved.</span><div><a href="https://github.com/xkintaro">GitHub</a><a href="https://discord.gg/NSQk27Zdkv">Discord</a><a href="https://instagram.com/xkintaro">Instagram</a><a href="https://www.linkedin.com/in/mustafa-tasal/">Linkedin</a></div></footer>
      </section>
    </main>
  )
}
